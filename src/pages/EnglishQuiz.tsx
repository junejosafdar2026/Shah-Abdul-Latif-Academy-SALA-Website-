...
import { ENGLISH_TOPICS } from '../constants';
...
export default function EnglishQuiz() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-12">
        <h1 className="text-4xl font-bold text-stone-800 mb-4">English Grammar Topics</h1>
        <p className="text-stone-500 text-lg">Select a topic to start practicing your English skills.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredTopics.map((topic, i) => (
          ...
          >
            <Link 
              to={`/quiz/english/${topic.id}`}
              className="group flex items-center justify-between p-6 bg-white border border-stone-100 rounded-2xl shadow-sm hover:shadow-md hover:border-emerald-200 transition-all"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-400 uppercase tracking-widest font-medium">English Category</p>
                </div>
              </div>
              <ChevronRight size={20} className="text-stone-300 group-hover:text-emerald-600 transition-transform group-hover:translate-x-1" />
            </Link>
          </motion.div>
        ))}
      </div>
